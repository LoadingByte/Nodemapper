...
public class NodegeomRenderer implements Renderer {
    ...
    public void setContentFont(final Font contentFont) {
    ...
    public void setContentPaint(final Paint contentPaint) {
    ...
    public void setContentBackgroundPaint(final Paint contentBackgroundPaint) {
    ...
    public void setEditStroke(final BasicStroke editStroke) {
    ...
    public void setEditPaint(final Paint editPaint) {
    ...
    public void setBorderStroke(final BasicStroke borderStroke) {
    ...
    public void setLinkStroke(final BasicStroke linkStroke) {
    ...
    public void setLinkPaint(final Paint linkPaint) {
    ...
    public void setCreationLinkStroke(final BasicStroke creationLinkStroke) {
    ...
    public void setCreationLinkPaint(final Paint creationLinkPaint) {

        this.creationLinkPaint = creationLinkPaint;
    }

    @Override
    public Shape getContentShape(final RenderNode node, final Graphics2D g) {

        int width = 0;
        for (final String line : node.getContentLines()) {
            if (g.getFontMetrics(contentFont).stringWidth(line) > width) {
                width = g.getFontMetrics(contentFont).stringWidth(line);
            }
        }
        final int height = g.getFontMetrics(contentFont).getHeight() * node.getContentLines().length;

        return new Rectangle(node.getLocation().x - width / 2, node.getLocation().y - height / 2, width, height);
    }

    @Override
    public Shape getInnerShape(final RenderNode node, final Graphics2D g) {

        final Rectangle rect = getRenderShape(node, g).getBounds();
        final int x = (int) (rect.x + borderStroke.getLineWidth() / 2);
        final int y = (int) (rect.y + borderStroke.getLineWidth() / 2);
        final int width = (int) (rect.width - borderStroke.getLineWidth());
        final int height = (int) (rect.height - borderStroke.getLineWidth());

        return new Rectangle(x, y, width, height);
    }

    @Override
    public Shape getRenderShape(final RenderNode node, final Graphics2D g) {

        final Rectangle rect = getContentShape(node, g).getBounds();
    ...
    @Override
    public Shape getCompleteShape(final RenderNode node, final Graphics2D g) {
    ...
    public void drawNode(final Graphics2D g, final RenderNode node) {
        ...
        final Rectangle contentRect = getContentShape(node, g).getBounds();
        final Rectangle innerRect = getRenderShape(node, g).getBounds();

        g.setPaint(contentBackgroundPaint);
        g.fill(innerRect);

        final String[] lines = node.getContentLines();
        for (int line = 0; line < lines.length; line++) {
            g.setPaint(contentPaint);
            final int x = contentRect.x + contentRect.width / 2 - g.getFontMetrics(contentFont).stringWidth(lines[line]) / 2;
            final int y = (int) (contentRect.y + g.getFontMetrics(contentFont).getAscent() + line * contentRect.height / lines.length * 1.1);
        ...
        if (node.isEdit()) {
            ...
            for (int line = 0; line < lines.length; line++) {
                ...
                if (node.getEditCursor() >= cursor && node.getEditCursor() <= cursor + lines[line].length()) {
                    ...
                    final int y = (int) (contentRect.y + g.getFontMetrics(contentFont).getAscent() + line * contentRect.height / lines.length * 1.1);
                    String before = "";
                    final char[] chars = lines[line].toCharArray();
    ...
    @Override
    public void drawLink(final Graphics2D g, final RenderNode node1, final RenderNode node2) {

        final Rectangle rect1 = getContentShape(node1, g).getBounds();
        final Rectangle rect2 = getContentShape(node2, g).getBounds();
    ...
    @Override
    public void drawCreationLink(final Graphics2D g, final Point point1, final Point point2) {
